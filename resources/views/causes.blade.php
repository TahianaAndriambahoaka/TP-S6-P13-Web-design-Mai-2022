    ...
    <section id="services">
      @if (!isset($data))
        <div class="sectiontitle">
          <h1 class="heading">Voici les causes du réchauffement climatique</h1>
        </div>
        <ul class="nospace group grid-3 animate__animated animate__fadeIn">
          @for ($i = 0; $i < count($articles); $i++)
            <li class="one_third animate__animated animate__pulse">
              <article><a href="{{ '/causes/'.str_slug($articles[$i]->titre,'-').'-'.$articles[$i]->id }}"><i class="fas fa-question"></i></a>
                ...
                <footer><a href="{{ '/causes/'.str_slug($articles[$i]->titre,'-').'-'.$articles[$i]->id }}">Plus d'informations &raquo;</a></footer>
              </article>
            </li>
          @endfor
        </ul>
